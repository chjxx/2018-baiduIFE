...
export default class Table{
  ...
  addEventListener(){
    ...
    this.$table.addEventListener('click', e =>{
      if(e.target.tagName === 'I'){
        ...
        if($i.classList.contains('edit')){
          $span.focus();
        }else if($i.classList.contains('finish_show')){
          $span.blur();
        }
      }
    });
    ...
    this.$table.addEventListener('focus', e =>{
      let $span = e.target;
      let $i = $span.parentNode.querySelector('i.finish');

      if($i){
        $i.classList.remove('finish');
        $i.classList.add('finish_show');
      }
    ...
    this.$table.addEventListener('blur', e =>{
      ...
      if(!partten.test(innerText)){
        ...
        let $i = e.target.parentNode.querySelector('i.finish_show');
        if($i){
          $i.classList.remove('finish_show');
    ...
    if(data instanceof Array){
      ...
      if(head){
        if(data.options instanceof Array){
          ...
          return `<tr><th>${data.val}</th>${data.sale.reduce((str, next) => `${str}<td><span contentEditable='true'>${next}</span><i class="edit">编辑</i><i class="finish">完成</i></td>`, '')}</tr>`;
        }
      }else{
        if(data.options instanceof Array){
          return `<th rowspan="${data.options.length}">${data.val}</th>${roll({data: data.options})}`;
        }else if(data.sale instanceof Array){
          return `<th>${data.val}</th>${data.sale.reduce((str, next) => `${str}<td><span contentEditable='true'>${next}</span><i class="edit">编辑</i><i class="finish">完成</i></td>`, '')}`;
        }
      }
    }
  }
